...
class Cache:
    ...
    def print_section(self, start, amount):
        ...
        for i in range(start, start + amount):
            print(dec_str(i, line_len) + ": " +
                  dec_str(self._lines[i].use, use_len) + " " +
                  bin_str(self._lines[i].modified, 1) + " " +
                  bin_str(self._lines[i].valid, 1) + " " +
                  bin_str(self._lines[i].tag, tag_len) + " <" +
                  " ".join([hex_str(i, 2) for i in self._lines[i].data]) + " @ " +
                  bin_str(self.get_physical_address(i), address_len) + ">")
        print()
